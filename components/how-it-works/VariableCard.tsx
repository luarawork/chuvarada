...
// motion.div cuida só da animação de entrada -- o Card (shadcn) por baixo
// cuida do visual. Não dá pra usar <Card> como o elemento animado
// diretamente porque Card não repassa initial/animate/transition (não é um
// motion component).
export function VariableCard({ emoji, title, weight, description }: VariableCardProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <Card
        className="rounded-2xl border backdrop-blur-sm shadow-none"
